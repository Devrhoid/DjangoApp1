...
class PollMethodTests(TestCase):
    ...
    def test_was_published_with_old_poll(self):
        ...
        """
        old_poll = Poll(pud_date=timezone.now()-datetime.timedelta(days=30))
    ...
    def create_poll(question,days):
        ...
        possitive for polls that have yet to be published)
        """
...
class PollViewTests(TestCase):
    ...
    def test_index_with_a_future_poll(self):
        ...
        response = self.client.get(reverse('polls:index'))
        self.assertQuerysetEqual(response,"No polls are available.",status_code=200)
        self.assetQuerysetEqual(response.context['latest_poll_list'],[])
